/* main.c  -  Lua library tools  -  Public Domain  -  2013 Mattias Jansson / Rampant Pixels
 * 
...
static NOINLINE int lua_dump_writer( lua_State* state, const void* buffer, size_t size, void* user_data )
{
	...
	if( !dump->bytecode )
		dump->bytecode = ( dump->bytecode ? memory_reallocate( dump->bytecode, dump->bytecode_size + size, 0 ) : memory_allocate( size, 0, MEMORY_PERSISTENT ) );
